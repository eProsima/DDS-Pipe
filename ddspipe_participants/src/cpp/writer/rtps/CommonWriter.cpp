    ...
    if (!come_from_this_participant_(info.remoteEndpointGuid))
    {
        if (info.status == fastrtps::rtps::MatchingStatus::MATCHED_MATCHING)
        {
            logInfo(DDSPIPE_RTPS_COMMONWRITER_LISTENER,
                    "Writer " << *this << " in topic " << topic_.serialize() << " matched with a new Reader with guid " << info.remoteEndpointGuid);
        }
        else
        {
            logInfo(DDSPIPE_RTPS_COMMONWRITER_LISTENER,
                    "Writer " << *this << " in topic " << topic_.serialize() << " unmatched with Reader " << info.remoteEndpointGuid);
        }
    }
}
