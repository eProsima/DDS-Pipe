...
/*!
 * @file hello_worldTypeObject.cpp
...
void registerhello_worldTypes()
{
    static std::once_flag once_flag;
    std::call_once(once_flag, []()
            {
                TypeObjectFactory *factory = TypeObjectFactory::get_instance();
                factory->add_type_object("hello_world", Gethello_worldIdentifier(true),
                Gethello_worldObject(true));
                factory->add_type_object("hello_world", Gethello_worldIdentifier(false),
                Gethello_worldObject(false));
...
const TypeIdentifier* Gethello_worldIdentifier(bool complete)
{
    const TypeIdentifier * c_identifier = TypeObjectFactory::get_instance()->get_type_identifier("hello_world", complete);
    if (c_identifier != nullptr && (!complete || c_identifier->_d() == EK_COMPLETE))
    {
        return c_identifier;
    }

    Gethello_worldObject(complete); // Generated inside
    return TypeObjectFactory::get_instance()->get_type_identifier("hello_world", complete);
}

const TypeObject* Gethello_worldObject(bool complete)
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("hello_world", complete);
    ...
    {
        return GetCompletehello_worldObject();
    }
    //else
    return GetMinimalhello_worldObject();
}

const TypeObject* GetMinimalhello_worldObject()
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("hello_world", false);
    if (c_type_object != nullptr)
    {
        return c_type_object;
    }
    ...
    TypeObjectFactory::get_instance()->add_type_object("hello_world", &identifier, type_object);
    delete type_object;
    return TypeObjectFactory::get_instance()->get_type_object("hello_world", false);
}

const TypeObject* GetCompletehello_worldObject()
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("hello_world", true);
    ...
    // Header
    type_object->complete().struct_type().header().detail().type_name("hello_world");
    // TODO inheritance
    TypeIdentifier identifier;
    ...
    TypeObjectFactory::get_instance()->add_type_object("hello_world", &identifier, type_object);
    delete type_object;
    return TypeObjectFactory::get_instance()->get_type_object("hello_world", true);
}
